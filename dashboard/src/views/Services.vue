...
const openEditDialog = async (service: Service) => {
  ...
  // Load applications and nodes
  await fetchApplications();
  await fetchNodes();

  // Find and select the application and node
  const app = applications.value.find(a => a.id === service.app_id);
  const node = nodes.value.find(n => n.id === service.node_id);

  if (app) {
    selectedApplication.value = app;
    // Initialize QA values if available
  ...
  // If both app and node are selected, go to step 3 directly for editing
  if (app && node) {
    await goToStep(3);
  } else {
    // Initialize first step data
...
<template>
  <div class="space-y-6">
    ...
    <!-- Loading State -->
    <div v-if="loading && services.length === 0" class="text-center py-8 text-muted-foreground">
      Loading...
    </div>
    ...
    <div v-else-if="services.length > 0" class="rounded-lg border bg-card">
      ...
      <!-- Pagination Controls -->
      <div v-if="totalPages > 1" class="flex items-center justify-between border-t px-6 py-4">
    ...
    <div v-else class="rounded-lg border bg-card p-12 text-center">
      <Icon icon="lucide:rocket" class="h-12 w-12 mx-auto text-muted-foreground mb-4" />
    ...
    <Sheet v-model:open="isDeployDialogOpen">
      <SheetContent class="flex h-full w-full max-w-[90vw] sm:max-w-none lg:max-w-[1200px] flex-col overflow-y-auto">
        <SheetHeader class="px-3 sm:px-5">
          <SheetTitle>{{ editingServiceId ? "Edit Service" : "Deploy Service" }}</SheetTitle>
          <SheetDescription>
            {{ editingServiceId ?
        ...
        <div class="flex-1 overflow-y-auto">
          <div class="px-3 sm:px-5 py-6">
            <Stepper v-model="currentStep" class="flex w-full items-start gap-2">
              ...
              <StepperItem v-slot="{ state }" class="relative flex w-full flex-col items-center justify-center"
                :step="1">
                <StepperSeparator
            ...
            <div class="mt-8 space-y-4">
              ...
              <div v-if="currentStep === 1" class="space-y-4">
                ...
                <div class="space-y-2">
                  ...
                  <div v-if="applicationsLoading" class="flex items-center justify-center py-8 text-muted-foreground">
                    <Icon icon="lucide:loader-2" class="h-5 w-5 mr-2 animate-spin" />
                    Loading applications...
                  </div>
                  <Select v-else id="deploy-app-select" :model-value="selectedApplication?.id?.toString()"
                    @update:model-value="
                    ...
                    <SelectContent>
                      <SelectItem v-for="app in applications" :key="app.id" :value="app.id.toString()">
                        {{ app.display || app.name }}
                        <span v-if="app.version" class="text-muted-foreground ml-2">
                          ({{ app.version }})
                        </span>
                      </SelectItem>
                    </SelectContent>
                  </Select>
                  <p v-if="selectedApplication?.description" class="text-sm text-muted-foreground">
              ...
              <div v-if="currentStep === 2" class="space-y-4">
                ...
                <div v-else class="space-y-4">
                  <div v-for="item in selectedApplication?.qa || []" :key="item.name" class="space-y-2">
                    <Label :for="`qa-${item.name}`">
                      {{ item.name }}
                      <span v-if="item.required" class="text-destructive">*</span>
                    </Label>
                    <p v-if="item.description" class="text-xs text-muted-foreground mb-2">
                      {{ item.description }}
                    </p>

                    <!-- Text Input -->
                    <Input v-if="item.type === 'text'" :id="`qa-${item.name}`" :model-value="getQAValue(item.name)"
                    ...
                    <!-- Textarea -->
                    <textarea v-else-if="item.type === 'textarea'" :id="`qa-${item.name}`"
                      ...
                      :value="getQAValue(item.name)" :placeholder="item.description || `Enter ${item.name}`"
                      @input="(e: Event) => handleQAValueChange(item.name, (e.target as HTMLTextAreaElement).value)" />

                    <!-- Number Input -->
                    <Input v-else-if="item.type === 'number'" :id="`qa-${item.name}`" type="number"
                      :model-value="getQAValue(item.name)" :placeholder="item.description || `Enter ${item.name}`"
                      @update:model-value="(val: string | number) => handleQAValueChange(item.name, String(val))" />

                    <!-- Boolean Checkbox -->
                    <div v-else-if="item.type === 'boolean'"
                      class="flex items-center space-x-3 rounded-md border px-3 py-2">
                      <Checkbox :id="`qa-${item.name}`" :model-value="getQAValue(item.name) === 'true'"
                    ...
                    <!-- Select -->
                    <Select v-else-if="item.type === 'select'" :model-value="getQAValue(item.name)" @update:model-value="
                      (val: any) =>
                        handleQAValueChange(item.name, val ? String(val) : '')
                    ">
                      <SelectTrigger>
                        <SelectValue :placeholder="item.description || `Select ${item.name}`
                          " />
                      </SelectTrigger>
                      <SelectContent>
              ...
              <div v-if="currentStep === 3" class="space-y-4">
                <div class="space-y-2">
                  ...
                  <div v-if="nodesLoading" class="flex items-center justify-center py-8 text-muted-foreground">
                    <Icon icon="lucide:loader-2" class="h-5 w-5 mr-2 animate-spin" />
                    Loading nodes...
                  </div>
                  <Select v-else id="deploy-node-select" :model-value="selectedNode?.id?.toString()"
                    @update:model-value="
        ...
        <SheetFooter class="flex flex-row items-center justify-end gap-2 px-3 sm:px-5">
          <Button variant="outline" @click="closeDeployDialog" :disabled="deployLoading">
            Cancel
          </Button>
          <Button v-if="currentStep > 1" @click="goToStep(currentStep - 1)" :disabled="currentStep == 1">
            <Icon icon="lucide:chevron-left" class="h-4 w-4 mr-2" />
            Previous
          </Button>
          <Button v-if="currentStep < 3" @click="goToStep(currentStep + 1)" :disabled="!canProceedToNextStep">
            Next
            <Icon icon="lucide:chevron-right" class="h-4 w-4 ml-2" />
          </Button>
          <Button v-if="currentStep === 3" @click="deployService({ onlySave: true })"
            :disabled="!canProceedToNextStep || deployLoading">
            <Icon icon="lucide:save" class="h-4 w-4 mr-2" />
            {{ editingServiceId ? "Update" : "Save" }}
          </Button>
          <Button v-if="currentStep === 3 && !editingServiceId" @click="deployService"
            :disabled="!canProceedToNextStep || deployLoading">
            <Icon v-if="deployLoading" icon="lucide:loader-2" class="h-4 w-4 mr-2 animate-spin" />
          ...
          <Button v-if="currentStep === 3 && editingServiceId" @click="deployService"
            ...
            <Icon v-if="deployLoading" icon="lucide:loader-2" class="h-4 w-4 mr-2 animate-spin" />
            <Icon v-else icon="lucide:cloud-upload" class="h-4 w-4 mr-2" />
            {{ deployLoading ? "Deploying..." : "Update & Deploy" }}
          </Button>
        </SheetFooter>
      </SheetContent>
    </Sheet>
