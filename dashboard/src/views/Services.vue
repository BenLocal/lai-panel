...
const handleStepChange = async (step: number) => {
    ...
    if (previousStep !== step) {
        if (step === 1 && applications.value.length === 0 && !applicationsLoading.value) {
...
const deployService = async () => {
    ...
    try {
        // TODO: Call deploy API when it's available
        // const response = await serviceApi.deploy({
        ...
        // For now, just close the dialog and refresh services
        closeDeployDialog();
        await fetchServices();
    } catch (error) {
        ...
        <Sheet v-model:open="isDeployDialogOpen">
            <SheetContent
                    ...
                    <div class="px-3 sm:px-5 py-6">
                        ...
                        <div class="mt-8 space-y-4">
                            ...
                            <div v-if="currentStep === 1" class="space-y-4">
                                <div class="space-y-2">
                                    ...
                                    <Select v-else id="deploy-app-select"
                                        ...
                                        @update:model-value="(val) => handleApplicationSelect(val?.toString() || '')">
                                        <SelectTrigger>
                            ...
                            <div v-if="currentStep === 2" class="space-y-4">
                                <div v-if="!selectedApplication?.qa || selectedApplication.qa.length === 0"
                                ...
                                <div v-else class="space-y-4">
                                    <div v-for="item in selectedApplication?.qa || []" :key="item.name"
                                        ...
                                        <Select v-else-if="item.type === 'select'" :model-value="getQAValue(item.name)"
                                            @update:model-value="(val) => handleQAValueChange(item.name, val ? String(val) : '')">
                                            <SelectTrigger>
                            ...
                            <div v-if="currentStep === 3" class="space-y-4">
                                <div class="space-y-2">
                                    ...
                                    <Select v-else id="deploy-node-select" :model-value="selectedNode?.id?.toString()"
                                        @update:model-value="(val) => handleNodeSelect(val?.toString() || '')">
                                        <SelectTrigger>
                ...
                <SheetFooter class="flex flex-row items-center justify-end gap-2 px-3 sm:px-5">
                    ...
                    <Button v-else @click="deployService" :disabled="!canProceedToNextStep || deployLoading">
