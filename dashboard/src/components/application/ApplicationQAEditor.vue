...
const normalizeQAItem = (item: ApplicationQAItem): ApplicationQAItem => {
  ...
  if (normalized.type === "select") {
    ...
    if (normalized.defaultValue && !options.includes(normalized.defaultValue)) {
      normalized.defaultValue = options[0] ?? "";
    }
  } else if (normalized.type === "boolean") {
    normalized.value = normalized.value === "true" ? "true" : "false";
    normalized.defaultValue =
      normalized.defaultValue === "true" ? "true" : "false";
...
const updateItemField = <K extends keyof ApplicationQAItem>(
  ...
  if (key === "type") {
    ...
    if (newType === "select") {
      ...
      nextItem.defaultValue = options[0] ?? "";
    } else if (newType === "boolean") {
      nextItem.options = undefined;
      nextItem.value = current.value === "true" ? "true" : "false";
      nextItem.defaultValue =
        current.defaultValue === "true" ? "true" : nextItem.value;
    } else {
      nextItem.options = undefined;
    }
  }
  ...
  if (key === "options") {
    ...
    if (
      nextItem.defaultValue &&
      !options.includes(nextItem.defaultValue ?? "")
    ) {
      nextItem.defaultValue = options[0] ?? "";
    }
  }
...
type QAStringField = "name" | "defaultValue" | "value" | "description";
...
const handleStringFieldInput = (
  ...
  updateItemField(index, key, target.value as ApplicationQAItem[QAStringField]);
};
    ...
    >
      ...
      <div class="grid gap-4 md:grid-cols-2">
        <div class="space-y-2">
          ...
          <Input
            ...
            @input="(event: Event) => handleStringFieldInput(index, 'name', event)"
          />
        </div>
      ...
      <div class="grid gap-4 md:grid-cols-2">
        <div class="space-y-2">
          ...
          <Input
            ...
            :value="qa.defaultValue ?? ''"
            placeholder="Default value"
            @input="(event: Event) =>
              handleStringFieldInput(index, 'defaultValue', event)"
          />
          <Select
            v-else-if="qa.type === 'boolean'"
            :model-value="qa.defaultValue ?? 'false'"
            @update:model-value="
              (value) => handleSelectFieldChange(index, 'defaultValue', value)
            "
          >
            <SelectTrigger>
        ...
        <div class="space-y-2">
          ...
          <div class="flex h-10 items-center space-x-3 rounded-md border px-3">
            <Checkbox
              ...
              :checked="qa.required ?? false"
              @update:checked="(checked: boolean) =>
                updateItemField(index, 'required', checked)"
            />
            <label
              :for="`qa-required-${index}`"
      ...
      <div class="space-y-2">
        ...
        <textarea
          ...
          @input="(event: Event) =>
            handleStringFieldInput(index, 'description', event)"
        ></textarea>
      </div>
      ...
      <div class="space-y-2">
        ...
        <Input
          ...
          @input="(event: Event) =>
            handleStringFieldInput(index, 'value', event)"
        />
        <Select
          v-else-if="qa.type === 'boolean'"
          :model-value="qa.value"
          @update:model-value="
